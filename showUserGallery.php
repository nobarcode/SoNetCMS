...
//link back
$showGalleryLinkBack = "<div id=\"link_back\"><a href=\"/showProfile.php?username=$username\">back to profile</a></div>\n";
...
$result = mysql_query("SELECT id FROM imagesUsers WHERE parentId = '{$username}' AND inSeriesImage = 1");
...
$result = mysql_query("SELECT parentId, id, imageUrl FROM imagesUsers WHERE parentId = '{$username}' AND inSeriesImage = 1 ORDER BY weight ASC LIMIT 3");
...
$result = mysql_query("SELECT id, imageUrl FROM imagesUsers WHERE parentId = '{$username}' AND inSeriesImage = 1 ORDER BY weight ASC LIMIT 1");
$row = mysql_fetch_object($result);


$_css_load =<<< EOF
@import url("/assets/core/resources/css/main/showUserGallery.css");
EOF;
...
<script language="javascript" src="/assets/core/resources/javascript/showUserGallery.js"></script>
						...
						<div>
							...
							<div id="comment_input_container">
								...
								<div class="add_comment_container_body">
									...
									<input type="hidden" id="parentId" name="parentId" value="$username">
									<input type="hidden" id="id" name="id" value="$row->id">
									<input type="hidden" id="type" name="type" value="userImageComment">
									</form>
								</div>
							</div>
						</div>
