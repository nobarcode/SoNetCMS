...
This is a special version of FileManager created to meet the needs of SoNet. It
remains opensource per the license agreement of the original version.

This version of FileManager removes a few options (e.g. thumbnails are always
created and displayed) but also adds new options and features as well as
requirements that didn't / don't exist in the original version. Of course, it
also fixes some bugs (and probably introduces a few as well). This version was
designed with PHP's capabilities in mind (mainly image manipulation for the
thumbnails) therefore the original FileManager's existing connectors are not
included. There were many additions to the language file so only the
English file is currently up-to-date.

If you are able to translate and update the language file, please do!
...
git clone https://github.com/no-barcode/Filemanager.git ** not alive anymore - sorry.

or download the archive from Github : https://github.com/no-barcode/Filemanager/zipball/master
...
(3) Find the default configuration file for the connector you chose in Step 2 above, and follow the same procedure to configure the connector. For instance, the default configuration file for the PHP connector is located here:
    Currently, PHP is the only connector available for this fork of Filemanager, but we are waiting for your contributions.
...
(4d) If you are integrating the FileManager as a standalone file manager, you should:

Pass FileManager the name of the callback function you want it to pass the selected file's path to by passing the
"callbackFunction" parameter the name of the function to call in parent window. You also need to pass it the name of an
element you want it to update, and it will pass that value back to the callbackFunction. Example:

function openFileManager(callbackFunction, elementName) {

	openServerBrowser(
	
		'/my/path/to/filemanager/index.html?callbackFunction=' + callbackFunction + '&elementName=' + elementName,
		screen.width * 0.7,
		screen.height * 0.7
...
function selectPath(elementName, path) {
	...
	$(elementName).setValue(path);
	
}

(5a) Settings options : Only display images and allow images uploads:

	PHP sample, set $config['upload']['imagesonly'] to true into the connector config file.
...
This fork of FileManager currently includes a connector for PHP in the following locations:
