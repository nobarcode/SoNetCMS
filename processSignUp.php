...
if ($error != 1) {
	...
	if ($result && $config->readValue('requireSignUpApproval') == 'false') {
		...
		while ($row = mysql_fetch_object($result)) {
			...
			mysql_query("INSERT INTO messages (dateSent, toUser, fromUser, subject, body, status, system) VALUES ($time, '{$row->username}', '{$username}', '" . sanitize_string($subject) . "', '" . sanitize_string($notificationText) . "', 'unread', 1)");
