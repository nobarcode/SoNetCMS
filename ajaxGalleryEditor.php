...
changeDirection($s, $d, $id);

function changeDirection($s, $d, $id) {
	...
	$result = mysql_query("SELECT * FROM imagesDocuments WHERE parentId = '{$id}' ORDER BY weight");
	...
	$result = mysql_query("SELECT * FROM imagesDocuments WHERE parentId = '{$id}' ORDER BY weight LIMIT $s, $per_page");
