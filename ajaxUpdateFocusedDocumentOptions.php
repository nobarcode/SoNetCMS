...
if ($error != 1) {
	...
	$result = mysql_query("UPDATE focusedDocuments SET dateStarts = '{$start_date}', dateExpires = '{$expire_date}' WHERE id = '{$id}'");
	...
	header('Content-type: application/javascript');
	print "$('#message_box').html('<div>Focused document parameters updated successfully.</div>');";
	print "$('#message_box').show();";
	print "regenerateList();";
	print "cancelEditFocusedDocumentOptions();";
