...
$result = mysql_query("SELECT featuredDocuments.id, featuredDocuments.activeState, documents.shortcut, documents.category, documents.subcategory, documents.subject, documents.title, DATE_FORMAT(featuredDocuments.dateStarts, '%m/%d/%Y %h:%i %p') AS newDateStarts, DATE_FORMAT(featuredDocuments.dateExpires, '%m/%d/%Y %h:%i %p') AS newDateExpires FROM featuredDocuments INNER JOIN documents ON documents.id = featuredDocuments.id WHERE 1$documentTypeFilter$categoryFilter$subcategoryFilter$subjectFilter ORDER BY weight");
...
if ($count == 0) {
	...
	print "<div class=\"featured_document_container\"><table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"100%\"><tr><td>No featured documents have been assigned.</td></tr></table></div>";
	exit;
...
while ($row = mysql_fetch_object($result)) {
	...
	if ($_SESSION['userLevel'] == 1 || $_SESSION['userLevel'] == 2 || $_SESSION['userLevel'] == 3) {
		...
		$showStateOptions = "<a href=\"javascript:toggleActiveState('$row->id');\" onclick=\"return confirm('Are you sure you want to change the status of this featured document?');\"><span id=\"active_state_$row->id\">$row->activeState</span></a>";
	...
	print "<div id=\"featured_document_$row->id\" class=\"featured_document_container\">";
	...
	if ($_SESSION['userLevel'] == 1 || $_SESSION['userLevel'] == 2 || $_SESSION['userLevel'] == 3) {
		...
		print "<td nowrap><div class=\"toolbar\"><div class=\"state\">$showStateOptions</div><div class=\"options\"><a href=\"javascript:initEditFeaturedDocumentOptions('$row->id');\">Options</a></div><div class=\"remove\"><a href=\"javascript:removeFeaturedDocument('$row->id');\" onClick=\"return confirm('Are you sure you want to remove this document from the featured list?');\">Remove</a></div></div></td>";
