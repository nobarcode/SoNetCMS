...
foreach($deleteId as $id) {
	...
	$result = mysql_query("SELECT conversations.groupId FROM conversations INNER JOIN groups ON groups.id = conversations.groupId WHERE conversations.id = '{$id}' LIMIT 1");
	...
	if ($_SESSION['userLevel'] == 1 || $_SESSION['userLevel'] == 2 || $isGroupAdmin == 1) {
		...
		mysql_query("DELETE conversations, conversationsPosts FROM conversations LEFT JOIN conversationsPosts ON conversationsPosts.parentId = conversations.id WHERE conversations.id = '{$id}'");
