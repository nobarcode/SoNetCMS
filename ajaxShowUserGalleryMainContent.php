...
$result = mysql_query("SELECT *, DATE_FORMAT(dateCreated, '%M %d, %Y %h:%i %p') AS newDateCreated FROM imagesUsers WHERE parentId = '{$parentId}' AND id = '{$imageId}' AND inSeriesImage = 1 ORDER BY weight ASC LIMIT 1");
...
$showImage = "<div class=\"document_image_container\"><img id=\"main_image\" src=\"/file.php?load=$row->imageUrl&w=720\" border=\"0\"></div>";
