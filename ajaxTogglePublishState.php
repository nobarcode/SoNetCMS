...
if ($error != 1) {
	...
	$result = mysql_query("SELECT category, publishState FROM documents WHERE id = '{$id}' LIMIT 1");
	...
	if ($row->publishState == 'Unpublished') { 
		...
		mysql_query("UPDATE documents SET datePublished = '{$time}', publishState = 'Published' WHERE id = '{$id}'");
		...
		mysql_query("UPDATE documents SET publishState = 'Unpublished' WHERE id = '{$id}'");
