...
if (!is_array($assigned) || trim($category) == "") {$error = 1;}

if ($error != 1) {
	
	//exit if the category has user groups assigned to it and the current user is not a member of any of those groups (and not a master account)
	...
	for ($x = 0; $x < count($assigned); $x++) {
		...
		mysql_query("DELETE FROM categoriesUserGroups WHERE groupId = '{$assigned[$x]}' AND category = '{$category}'");
